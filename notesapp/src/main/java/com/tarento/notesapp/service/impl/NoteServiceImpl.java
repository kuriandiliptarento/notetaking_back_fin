...
public class NoteServiceImpl implements NoteService {
    ...
    public NoteResponseDto createNote(NoteRequestDto request) {
        ...
        // Attach tags via NoteTag join entities (with defensive check to avoid duplicates)
        if (!resolvedTags.isEmpty()) {
            for (Tag tag : resolvedTags) {
                // defensive: skip if note already contains this tag in-memory
                boolean exists = note.getNoteTags().stream()
                        .map(nt -> nt.getTag() == null ? null : nt.getTag().getId())
                        .anyMatch(id -> Objects.equals(id, tag.getId()));
                if (exists) continue;
    ...
    public NoteResponseDto updateNote(Long id, NoteRequestDto request) {
        ...
        // Update tags: Remove tags not in the request, add new ones
        Set<Long> requestedTagIds = resolvedTags.stream()
                .map(Tag::getId)
                .collect(Collectors.toSet());
        
        // Get currently existing tag IDs
        Set<Long> existingTagIds = note.getNoteTags().stream()
                .map(nt -> nt.getTag().getId())
                .collect(Collectors.toSet());
        
        // Remove NoteTags for tags that are no longer in the request
        note.getNoteTags().removeIf(nt -> !requestedTagIds.contains(nt.getTag().getId()));
        
        // Add new tags (only those that don't already exist)
        for (Tag tag : resolvedTags) {
